...
RSpec.describe Wip::CommandBuilder do
  ...
  it "builds interactive run commands with environment, ports, and volumes" do
    settings = { "interactive" => true, "env" => { "PORT" => "3000" }, "ports" => ["3000:3000"], "volumes" => [".:/app"] }
    expect(builder.run(["server"], settings: settings)).to eq(["wslc.exe", "run", "--rm", "-it", "-w", "/app", "-e", "PORT=3000", "-p", "3000:3000", "-v", ".:/app", "example:dev", "server"])
  end

  it "builds images with extra options before context" do
    expect(builder.build(settings: config.command("build"), extra: ["--no-cache"])).to eq(%w[wslc.exe build -t example:dev --no-cache .])
  end
