...
module Wip
  class Doctor
    ...
    def check_version(command, results)
      ...
      results << Result.new(status.success? ? :ok : :fail, status.success? ? "WSLC is available" : "WSLC version failed")
    rescue Errno::ENOENT
      results << Result.new(:fail, "WSLC version failed")
    end

    def result(condition, yes, no) = Result.new(condition, condition == :ok ? yes : no)
